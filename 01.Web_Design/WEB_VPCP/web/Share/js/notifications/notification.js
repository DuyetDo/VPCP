/* global VitxUtils, group, moment, zk, zAu */
...
var notificationGroup = function (objectId, objectType, currentUserId, classView) {
    ...
    var $td = $(pefix.trim() + '.documentViewTable').children('tbody').children('tr').children('td');
    this.notificationContainer = $(pefix + "#notificationContainer");
    var heightNotify = $td.height();
...
notificationGroup.prototype = {
    ...
    resetNotificationDiv: function () {
//        var $list = this.$rootNotificationList.children("div");
...
//        return;
    ...
    drawSubNotifications: function (parentId, subNotifications, $subNotificationList) {
    ...
    createCommentContent: function (notification) {
        ...
        var $commentBodySpan = $("<span>", {
            ...
            html: notification.content ? this.XSSEncode(notification.content) : "&nbsp;"
        });
        ...
        var numberUser = notification.receiver.match(/(deptId:null)/g) ? notification.receiver
                ...
                : 0;
        var editable = notification.editable;
        // console.log(notification);
        var self = this;
        if (numberUser === 0) {
            ...
            if (visibleReceiverInfo) {
                var $numberOfReceiverA = $("<a>", {
                    html: numberOfReceivers + ' người nhận'
                });
                ...
                $numberOfReceiverA.hover(function (event) {
                    ...
                    var $receiverList = $commentActionsDiv.children('div.receiverContainer');
        ...
        var $editAction = $("<a>", {html: "Xóa", "style": "color: #CC0000;"});
        $editAction.on('click', function () {
            ...
            if (notifyId > 0) {
                var answer = confirm("Bạn có chắc chắn muốn xóa ý kiến không?");
                if (answer === true) {
                    self.deleteComment(notification.notifyId, self.toIndex, notification.receiverUserId,
                            notification.receiverRoleId, notification.receiverDeptId);
                    self.toIndex = self.toIndex - 1;
                    var $replyList = group.checkExistsSubNotifications(notifyId);
                    if ($replyList)
                        $replyList.remove();
                    var $infoNotify = group.checkExistsInfoNotifications(notifyId);
                    if ($infoNotify)
                        $infoNotify.remove();
                    $(".UFIPagerCount").first().html((self.toIndex - self.fromIndex) + " / " + self.toIndex).change().show();
                    return;
                } else {
                    return;
                }
        ...
        if (editable) {
            $commentActionsDiv.append($editAction);
            $commentActionsDiv.append($("<span>", {
                html: "&nbsp;&nbsp;&nbsp;&nbsp;"
            }));
        }
    ...
    createInputBox: function (notification) {
        ...
        $textarea.attr("placeholder", "Nhập nội dung và nhấn phím Enter để gửi").placeholder();
    ...
    sendComment: function (parentId, content, toIndex, sendUserId, sendRoleId, sendDeptId) {
        var data = {
            fromIndex: toIndex,
            parentId: parentId,
            content: content,
    ...
    deleteComment: function (notifyId, toIndex, sendUserId, sendRoleId, sendDeptId) {
        var data = {
            fromIndex: toIndex,
            notifyId: notifyId,
            receiverUserId: sendUserId,
        ...
        zAu.send(new zk.Event(group.wdNotificationSide, "onDeleteComment", data));
    },
    ...
    createViewMoreReplies: function (parentId, $subNotificationList) {
        ...
        var $text = $(
                "<span>",
                {
                    ...
                    html: $subNotificationList.find('.UFIRow.UFIComment').length > 0 ? 'Ẩn ý kiến'
                            : 'Hiển thị ý kiến'
                });
    ...
    refreshViewMoreNotifications: function () {
        if (this.fromIndex !== 0) {
            $(".UFIPagerLink>span").first().html('Hiển thị thêm ý kiến');
        } else {
    ...
    createReceiverList: function (data, isOpinion) {
        ...
        if (data.numberOfRestReceivers) {
            var $li = $("<li>", {
                html: 'và ' + data.numberOfRestReceivers + ' người nữa'
            });
            $ul.append($li);
        }
    ...
    checkExistsInfoNotifications: function (notifyId) {
        var $notificationRow = $('#notification_' + notifyId);
    ...
    drawEditNotificationsWithOnlyTextbox: function (notification) {
        var $subNotificationList = group.createSubNotificationList(
                notification, true);
        var $parentNotification = group.$rootNotificationList
                .find('#notification_' + notification.notifyId);
